...
clustering() {
    ...
    sub_scan = n.subscribe("scan", 1, &clustering::scanCallback, this);

    pub_moving_persons_detector_marker = n.advertise<visualization_msgs::Marker>("clustering", 1); // Preparing a topic to publish our results. This will be used by the visualization tool rviz
...
//then we end the current cluster with the previous hit and start a new cluster with the current hit
    ...
    /*for( int loop=1; loop<nb_beams; loop++ )//loop over all the hits
        ...
        {//the previous hit is the end of the current cluster
            ...
            //textual display
            ROS_INFO("cluster[%i]: hit[%i](%f, %f) -> hit[%i](%f, %f)", nb_cluster, start, current_scan[start].x, current_scan[start].y, end, current_scan[end].x, current_scan[end].y);
